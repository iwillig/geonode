...
def save(layer, base_file, user, overwrite = True, title=None, abstract=None, permissions=None, keywords = []):
    ...
    typename = "%s:%s" % (target.workspace, target.name)
    ...
    # @todo cannot save as layer does not really exist in geoserver yet
    ...
    return saved_layer, import_session
...
class TimeForm(forms.Form):
    ...
    def __init__(self, *args, **kwargs):
        ...
        if att_names:
            self.fields['time_attribute'] = forms.ChoiceField(choices=[ (a,a) for a in att_names])
...
def _create_time_form(req):
    ...
    att_names = [ att.name for att in feature_type.attributes ]
    if req.method == 'POST':
