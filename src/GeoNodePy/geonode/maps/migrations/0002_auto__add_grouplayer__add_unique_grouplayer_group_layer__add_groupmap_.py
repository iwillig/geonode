...
class Migration(SchemaMigration):
    ...
    def forwards(self, orm):
        ...
        # Adding model 'GroupLayer'
        db.create_table('maps_grouplayer', (
        ...
        db.send_create_signal('maps', ['GroupLayer'])

        # Adding unique constraint on 'GroupLayer', fields ['group', 'layer']
        db.create_unique('maps_grouplayer', ['group_id', 'layer_id'])

        # Adding model 'GroupMap'
        db.create_table('maps_groupmap', (
            ('id', self.gf('django.db.models.fields.AutoField')(primary_key=True)),
            ('group', self.gf('django.db.models.fields.related.ForeignKey')(to=orm['groups.Group'])),
            ('map', self.gf('django.db.models.fields.related.ForeignKey')(to=orm['maps.Map'])),
        ))
        db.send_create_signal('maps', ['GroupMap'])

        # Adding unique constraint on 'GroupMap', fields ['group', 'map']
        db.create_unique('maps_groupmap', ['group_id', 'map_id'])


    def backwards(self, orm):
        
        # Removing unique constraint on 'GroupMap', fields ['group', 'map']
        db.delete_unique('maps_groupmap', ['group_id', 'map_id'])

        # Removing unique constraint on 'GroupLayer', fields ['group', 'layer']
        db.delete_unique('maps_grouplayer', ['group_id', 'layer_id'])

        # Deleting model 'GroupLayer'
        db.delete_table('maps_grouplayer')

        # Deleting model 'GroupMap'
        db.delete_table('maps_groupmap')
    ...
    models = {
        ...
        'maps.grouplayer': {
            'Meta': {'unique_together': "(('group', 'layer'),)", 'object_name': 'GroupLayer'},
            'group': ('django.db.models.fields.related.ForeignKey', [], {'to': "orm['groups.Group']"}),
            'id': ('django.db.models.fields.AutoField', [], {'primary_key': 'True'}),
            'layer': ('django.db.models.fields.related.ForeignKey', [], {'to': "orm['maps.Layer']"})
        },
        'maps.groupmap': {
            'Meta': {'unique_together': "(('group', 'map'),)", 'object_name': 'GroupMap'},
            'group': ('django.db.models.fields.related.ForeignKey', [], {'to': "orm['groups.Group']"}),
            'id': ('django.db.models.fields.AutoField', [], {'primary_key': 'True'}),
